...
const TimesheetDateInput = ({
	...
	setTimesheetQueryDate: (date: Date) => void;
}) => {
	const onChange = (date: Date) => {
		setTimesheetQueryDate(
			DateTime.fromJSDate(date).startOf("day").toUTC().startOf("day").toJSDate()
		);
	};
	...
	const nextPeriod = () => {
		if (!startDate) return;
		setTimesheetQueryDate(
			DateTime.fromJSDate(timesheetQueryDate, { zone: "utc" })
	...
	const filterDate = React.useCallback(
		(date: Date) => {
			...
			return !!(
				Math.abs(dateTime.diff(startDate ?? DateTime.now(), "days").days) %
					(periodLength ?? 14) !==
				0
			);
		},
