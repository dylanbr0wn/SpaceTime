...
const TimesheetTable = ({
	...
	const mutation = trpc.useMutation(["timeEntryRow.create"], {
		...
		onError: (err, newEntry, context) => {
			if (!context?.previousTodo)
				utils.refetchQueries(["timeEntryRow.readAll"]);
			else {
				utils.setQueryData(
					[
						"timeEntryRow.readAll",
							...
							return (
								...
								>
									<div className={`${dayFeatures.style}`}>
										<div className="text-center">{date.toFormat("ccc")}</div>
						...
						{table.getHeaderGroups().map((headerGroup) => (
							<div className="flex space-x-0.5 " key={headerGroup.id}>
								{headerGroup.headers.map((header) => {
									if (header?.column?.parent?.id === "hours") {
										return (
											<div
										...
										return (
											<span className="w-12" key={header.id}>
												{null}
											</span>
										);
									} else if (header.column.id === "workdescription") {
										return (
							...
							{table.getRowModel().rows.length > 0 ? (
								<div className=" space-y-2">
									{table.getRowModel().rows.map((row) => {
										return (
											<div className="rounded-md flex space-x-0.5" key={row.id}>
												{row.getVisibleCells().map((cell) => {
													if (cell.column.id === "deleter") {
														return (
															<div className="w-12" key={cell.id}>
																{flexRender(
													...
													} else if (cell?.column?.parent?.id === "hours") {
														return (
															<div className="w-14 " key={cell.id}>
																{flexRender(
																	cell.column.columnDef.cell,
																	cell.getContext()
																)}
														...
														return (
															<div className="w-44 " key={cell.id}>
																{flexRender(
																	cell.column.columnDef.cell,
																	cell.getContext()
																)}
								...
								<div className="text-primary text-lg w-full text-center py-3">
									Looks like you have no rows... Click the button below to make
									one!
								</div>
							)}
						</>
					...
					<div>
						<div className="text-center mt-4">
							...
							>
								...
								<div className="text-sm font-bold my-auto">New Row</div>
							</button>
						</div>
					</div>
				</div>
