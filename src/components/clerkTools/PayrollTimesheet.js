		...
		<Container fluid className="userContainer bg-light">
			<ErrorBoundary>
				<Row>
					<Col>
						<Button
							onClick={() => history.push("/payrollTimesheetDashboard")}
					...
					<Col>
						{nextUserUp && (
							<Button
								...
								variant="link"
								onClick={() => {
									history.push(`/timesheet/${nextUserUp.EmployeeID}/payroll`);
								}}
							>
								<i
									style={{
								...
								/>
								{nextUserUp.FirstName + " " + nextUserUp.LastName}
							</Button>
						)}
					</Col>
					<Col>
						{nextUserDown && (
							<Button
								...
								variant="link"
								onClick={() => {
									history.push(`/timesheet/${nextUserDown.EmployeeID}/payroll`);
								}}
								style={{ color: "#233746" }}
							>
								{nextUserDown.FirstName + " " + nextUserDown.LastName}
								<i
			...
			>
				<div className="text-center">
					...
					<div>
						Your approval lock will soon expired and others will soon be able
						make changes. Please refresh the page to renew the session.
					</div>
				</div>
			</CustModal>
		</Container>
