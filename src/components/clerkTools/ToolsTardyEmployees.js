...
const ToolsTardyEmployees = ({
	...
	useEffect(() => {
		...
		if (newLoad) {
			readTardyEmployeesReportDispatch(new Date(settings.CutOffDate)).then(
				(result) => {
					if (result.success) {
						setTardyEmployees(
	...
	return (
		<>
			<Row>
				<Col>
					<p>
						Lists all employees that have not submitted any time on the current
						pay period.
					</p>
				</Col>
			</Row>
			<Row>
				<Col>
					...
					>
						Reload <i style={{ marginLeft: 5 }} className="fas fa-sync"></i>
					</Button>
				</Col>
			</Row>
			{!newLoad ? (
			...
			>
				{selectedEmployee.DepartmentID && (
					<>
						...
						<Row>
							<Col>
								<DescriptionItem header="Email" info={selectedEmployee.Email} />
							</Col>
						</Row>
						<Row>
							<Col>
								<DescriptionItem
									header="Department"
									info={getEmployeeDepartment(selectedEmployee.DepartmentID)}
								/>
							</Col>
						</Row>
						<Row>
						...
						<Row>
							<Col>
								<DescriptionItem header="Role" info={selectedEmployee.Role} />
							</Col>
							<Col>
								<DescriptionItem
									header="Status"
